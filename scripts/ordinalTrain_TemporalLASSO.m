function [OrdinalMod] = ordinalTrain_TemporalLASSO(Xs, Ys, rhos, opts)
    ...
    W = cell((length(labelSet) - 1), 1);
    c = cell((length(labelSet) - 1), 1);
    
    for i = 1:(length(labelSet) - 1)
        Ys_ = Ys;
        for n = 1:n_task
    ...
    OrdinalMod = struct('W', W, 'c', c, 'labelSet', labelSet);
end