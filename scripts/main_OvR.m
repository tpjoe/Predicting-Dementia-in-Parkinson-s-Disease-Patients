...
%     end
    ...
    if cv_method == "cv2"
        if model_type == "TGL"
            [OvRMod, p] = cv2_OvRTemporalLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        elseif model_type == "CFGL"
            [OvRMod, p] = cv2_OvRCFGLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        elseif model_type == "nFSGL"
            [OvRMod, p] = cv2_OvRnFSGLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        end
    else
        if model_type == "TGL"
            [OvRMod, p] = cv_OvRTemporalLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        elseif model_type == "CFGL"
            [OvRMod, p] = cv_OvRCFGLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        elseif model_type == "nFSGL"
            [OvRMod, p] = cv_OvRnFSGLASSO(Xs_trSet, Ys_trSet, cvfold, Rhos, cv_method, opts);
        end
    end
    ...
    Ys_pred_ = OvRPredict(Xs_teSet, OvRMod);
    Ys_pred(:, i) = Ys_pred_';
