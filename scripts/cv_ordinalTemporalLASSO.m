function [OrdinalMod_final, params] = cv_ordinalTemporalLASSO(Xs_trSet, Ys_trSet, ...
    cv_fold, Rhos, cv_method, opts)

    % The function returns OrdinalMod_final which is a {3 x n_class} struct
...
%             for visit = 1:length(cv_Yval)
%             display([tabulate(cv_Ytr{visit}); tabulate(cv_Yval{visit})])
%             end
            
            for rho = 1:mRho
                OrdinalMod = ordinalTrain_TemporalLASSO(cv_Xtr, cv_Ytr, ...
                    Rhos(rho, :), opts);
                mods{1, rho} = OrdinalMod;
                Ys_pred = ordinalPredict(cv_Xval, OrdinalMod);
                AUC = zeros(1, n_task);
    ...
    OrdinalMod_final = struct('W', W_final, 'c', c_final, 'labelSet', labelSet);
end
